...
def group_corr(population, gene_list):
    """Measures the correlation of all genes within a list to the average expression of all genes within that list (used for cell cycle position calling)
...
def refine_gene_list(population, gene_list, threshold, return_corrs=False):
    """Refines a list of genes by removing those that don't correlate well with the average expression of those genes
    ...
    threshold: threshold on correlation coefficient used to discard genes (expression of each gene is compared to the bulk expression of the group and any gene with a correlation coefficient less than this is discarded)
...
def group_score(population, gene_list):
    """Scores cells within population for expression of a set of genes. Raw expression data are first log transformed, then the values are summed, and then scores are Z-normalized across all cells.
...
def get_cell_phase_genes(pop, refine=False, threshold=0):
    ...
    refine: whether to refine the gene lists based on how consistent the expression is among the groups
...
def cell_cycle_position_heatmap(pop, cells=None, **kwargs):
    ...
    **kwargs: all other keyword arguments are passed to where

...
def strip_low_expression(pop, threshold=0):
    """ Remove genes with low or zero expression to reduce memory usage
    """
...
def log_normalize_expression(pop, scale_by_total=True, pseudocount=1):
    ...
    The normalization proceeds by first (optionally) normalizing the UMI counts within each cell to the median UMI count within the population. The expression within the population is then Z-normalized: i.e., for each gene the mean is subtracted, and then these values are divided by that gene's standard deviation.
...
def z_normalize_expression(pop, scale_by_total=True):
    """ Normalize expression distribution.
    The normalization proceeds by first normalizing the UMI counts within each cell to the median UMI count within the population. The expression within the population is then Z-normalized: i.e., for each gene the mean is subtracted, and then these values are divided by that gene's standard deviation.
...
def normalize_matrix_to_control(matrix, control_matrix, scale_by_total=True, median_umi_count=None):
    ...
    The normalization proceeds by first normalizing the UMI counts within each cell to the median UMI count within the population. The control population is normalized to the same amount. The expression within the population is then Z-normalized with respect to the control distribution: i.e., for each gene the control mean is subtracted, and then these values are divided by the control standard deviation.
    
    Example: 
    control_cells = pop.cells[pop.cells['perturbation']=='DMSO_control'].index.values
    pop.normalized_matrix = normalize_expression_to_control(pop.matrix, pop.matrix.loc[control_cells].copy())
    ...
    median_umi_count: If provided, set the median to normalize to. This is useful for example when normalizing multiple independent lanes.
    """
...
def normalize_to_gemgroup_control(pop, control_cells, median_umi_count=None, **kwargs):
    """Normalizes a multi-lane 10x experiment. Each gemgroup is normalized to the control
    cells within the same gemgroup.
    
    Example:
        normalized_matrix = normalize_to_gemgroup_control(pop, control_cells='guide_identity == "control"')
